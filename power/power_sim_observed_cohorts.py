...
COHORTS = [15] * 6 + [17] * 3 + [19] * 6 + [np.inf] * 5   # SENSITIVITY: proportions of the 7 NCBs collected so far, scaled to 20 [ASSUMPTION]
SD_UNIT_PERIOD, SD_PASSAGE = 0.15, float(np.hypot(0.5, 0.4))
N_PERM, N_REP = 199, 150
LAMBDAS = [0.5, 1.0, 2.0]
BETAS = [0.0, 0.2, 0.3, 0.45]
...
df.to_csv("power_grid_observed_cohorts.csv", index=False)
...
with open("mde_observed_cohorts.json", "w") as f:
